...
function MainContentJobs() {
  ...
  const getSingleProduct = async () => {
      ...
      if(dataCandidate){
      const SortedCampanyData = await axios.get("https://backend.am3dpjobs.com/filteredTallent", {
        params:{
          jobmode: dataCandidate.data.JobMode,
          typeofwork: dataCandidate.data.TypeOfWork,
          }
      })
      ...
      <div className='p-3 relative' style={{borderBottom:"2px solid #E5E5E5"}}>
        <div className='d-flex flex-column w-fullc' style={{cursor:"pointer"}} onClick={popup}>
          <div className='d-flex w-fullc'>
            ...
            {candidateData.JobSpecialisation}
               ...
               {candidateData.Location}</span></span></div>
          </div>
